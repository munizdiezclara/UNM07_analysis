...
```
```{r, include=FALSE}
demographics <- read_csv("prolific_export_UNM07.csv")
demographics <- filter(demographics, demographics$`Participant id` %in% check_with_demo$participant)
```
...
for (subj in 1:length(csv_files)) {
  ...
  pTraining <- pTraining %>%
    mutate(block = rep(1:8, each = 20), .after = gender)
...
training <- training %>% #add pNum
  mutate(pNum = rep(c(1:(nrow(training)/160)), each = 160), .after = participant) %>%
...
test <- test %>%
  ...
  mutate(condition = case_when(condition == 1 ~ "Certain",
                               condition == 2 ~ "Uncertain"))
not_passed_df <- filter(training, participant %in% not_passed)
not_passed_pNum <- not_passed_df$pNum
not_passed_pNum <- unique(not_passed_pNum)
training <- subset(training, select = -participant)
test <- subset(test, select = -participant)
```


```{r, include=FALSE}
save(training, test, demographics, not_passed_pNum, file = "UNM07_proc_data.RData")
```
