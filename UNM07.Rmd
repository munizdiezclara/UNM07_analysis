...
sme_mem_UNM07_test_pred <- c_mem_test %>%
  group_by(condition) %>%
  anova_test(mean_c_mem ~ predictiveness + Error(pNum/predictiveness), effect.size = "pes") %>%
  get_anova_table() %>%
  adjust_pvalue(method = "bonferroni")
sme_mem_UNM07_test_pred #Call the output table
#calculate the simple main effect of predictiveness
sme_mem_UNM07_test_cond<- c_mem_test %>%
  group_by(predictiveness) %>%
  anova_test(mean_c_mem ~ condition, effect.size = "pes") %>%
  get_anova_table() %>%
  adjust_pvalue(method = "bonferroni")
sme_mem_UNM07_test_cond #Call the output table
```
